...
select
	case 
		...
		when professional_status = 'busine' then 'negocios'
...
with faixa_de_idade_dos_veiculos as (
		select 
			...
			case
				...
				when (extract('year' from visit_page_date) - pro.model_year::int)<==4 then 'de 2 até 4 anos'
				when (extract('year	' from visit_page_date) - pro.model_year::int)<==6 then 'de 4 até 6 anos'
				when (extract('year' from visit_page_date) - pro.model_year::int)<==8 then 'de 6 até 8 anos'
				when (extract('year' from visit_page_date) - pro.model_year::int)<==10 then 'de 8 até 10 anos'
